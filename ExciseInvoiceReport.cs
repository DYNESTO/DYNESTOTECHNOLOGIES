...
{
    public partial class ExciseInvoiceReport : Form
    {
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CharityManagement"].ConnectionString);
        SqlCommand cmd;
        SqlDataAdapter adapt;
        public ExciseInvoiceReport()
        {
            InitializeComponent();
        }

        private void ExciseInvoiceReport_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            ...
            adapt = new SqlDataAdapter("Select * from ExciseInvoice_Table7 where ClientName='" + compare + "' and DocumentNo='" + compare1 + "'", con);
            adapt.Fill(dt);
        ...
        private void cmb_ClientName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            con.Open();
            DataTable dt = new DataTable();
            adapt = new SqlDataAdapter("Select * from ExciseInvoice_Table7 where ClientName='" + compare + "'", con);
            adapt.Fill(dt);
            dataGridView1.DataSource = dt;
            con.Close();
        }
        ...
        private void dataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                cmb_Payment.Text = dr.Cells[17].Value.ToString();
                txt_Total_Amount.Text = dr.Cells[23].Value.ToString();
                value4 = Convert.ToDecimal(dr.Cells[18].Value.ToString());
                txt_Amount_Due.Text = dr.Cells[24].Value.ToString();
        ...
        private void btn_Pay_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (cmb_Payment.Text != "" )
                    {
                        String query = "update ExciseInvoice_Table7 set PaymentType=@PaymentType,AmountPaid=@AmountPaid,AmountDue=@AmountDue where DocumentNo='" + compare + "'";
