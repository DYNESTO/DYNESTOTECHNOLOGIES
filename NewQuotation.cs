...
{
    public partial class NewQuotation_frm : Form
    {
        ...
        private void NewQuotation_frm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                while (reader.Read())
                {
                    txt_AddClient.SelectedItem = "Select";
                    txt_AddClient.Items.Add(reader["ClientName"].ToString());
            ...
            try
            {
                String query = "select * from Quotation_Table9";
        ...
        private void cmb_AddClient_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {

                string compare = txt_AddClient.Text;
        ...
        public void save()
        {
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                ...
                con.Open();
                cmd.Connection = con;
                cmd.CommandText = "insert_QuotationProduct";
        ...
        {
            if (txt_AddClient.Text.Trim() == string.Empty)
            {
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            if (isvalidated())
            {
                ...
                if (txt_AddClient.Text != "" || txt_DocNumber.Text != "" || txt_PONumber.Text != "" || dateTimePicker1.Text != "" || dateTimePicker2.Text != "" || cmb_ProductName.Text != "" || txt_Description.Text != "" || cmb_UOM.Text != "" || txt_QTY.Text != "" || txt_UnitPrice.Text != "" || cmb_Tax.Text != "" || txt_TotalDisc.Text != "" || txt_AddShipCharge.Text != "" || cmb_PayType.Text != "" || txt_AmountPaid.Text != "" || txt_Notes.Text != "" || txt_NFC.Text != "" || txt_PrivateNote.Text != "" || lbl_SubtotalAns.Text != "" || lbl_rs.Text != "" || txt_AmountDue.Text !="")
                {
                    cmd = new SqlCommand("insert into Quotation_Table9(DocumentNo,ClientName,PONumber,IssueDate,ValidDate,ProductName,Description,UOM,QTY,UnitPrice,Tax,TotalDiscount,ShippingCost,PaymentType,AmountPaid,Notes,NFC,PrivateNote,TotalAmount,SubTotal,AmountDue) values(@DocumentNo,@ClientName,@PONumber,@IssueDate,@ValidDate,@ProductName,@Description,@UOM,@QTY,@UnitPrice,@Tax,@TotalDiscount,@ShippingCost,@PaymentType,@AmountPaid,@Notes,@NFC,@PrivateNote,@TotalAmount,@SubTotal,@AmountDue)", con);
                    con.Open();

                    cmd.Parameters.AddWithValue("@DocumentNo", txt_DocNumber.Text.Trim());
                    cmd.Parameters.AddWithValue("@ClientName", txt_AddClient.Text.Trim());
                    ...
                    cmd.Parameters.AddWithValue("@ShippingCost", txt_AddShipCharge.Text.ToString());
        ...
        {
            QuotationPrint_frm frm = new QuotationPrint_frm();
            frm.Show();
        }
        ...
        private void btn_AddShipCost_Click(object sender, EventArgs e)
        {
            ...
            totalamountshipping = Convert.ToDecimal(txt_AddShipCharge.Text);
            ...
            txt_AddShipCharge.Text = "";
            btn_AddShipCost.Enabled = false;

        }

        private void txt_DocNumber_TextChanged(object sender, EventArgs e)
        {
            try
            {

                string compare = txt_DocNumber.Text;


                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CharityManagement"].ConnectionString);
                // SqlCommand cmd;

                string query = "select ClientName,PONumber,ProductName,TotalDiscount,ShippingCost,PaymentType,Notes,AmountPaid,AmountDue,SubTotal,TotalAmount  from Quotation_Table9 where DocumentNo='" + compare + "'";
                SqlCommand cmd = new SqlCommand(query, con);
                SqlDataReader reader;
                con.Open();
                reader = cmd.ExecuteReader();
                ...
                while (reader.Read())
                {
                    ...
                    txt_AddClient.Text = reader["ClientName"].ToString();
                    ...
                    txt_AddShipCharge.Text = reader["ShippingCost"].ToString();
        ...
        private void btn_Update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txt_AddClient.Text != "" || txt_TotalDisc.Text != "" || txt_AddShipCharge.Text != "" || cmb_PayType.Text != "" || txt_Notes.Text != "" || txt_AmountPaid.Text != "" || txt_AmountDue.Text != "" || lbl_rs.Text != "")
                {

                    String query = "update Quotation_Table9 set ClientName=@ClientName,TotalDiscount=@TotalDiscount,ShippingCost=@ShippingCost,PaymentType=@PaymentType,Notes=@Notes,AmountPaid=@AmountPaid,AmountDue=@AmountDue,TotalAmount=@TotalAmount where DocumentNo='" + compare + "'";
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string compare = txt_DocNumber.Text;
                con.Open();
                SqlDataAdapter sqldata = new SqlDataAdapter("delete from Quotation_Table9  where DocumentNo='" + compare + "'", con);
        ...
        private void txt_AddShipCharge_KeyPress(object sender, KeyPressEventArgs e)
        {
            if ((e.KeyChar >= 48 && e.KeyChar <= 57) || e.KeyChar == 8)
            {
                e.Handled = false;
        ...
        private void txt_AddShipCharge_TextChanged(object sender, EventArgs e)
        {
            if (txt_AddShipCharge.Text == "")
            {
                btn_AddShipCost.Enabled = false;
            }
            else
